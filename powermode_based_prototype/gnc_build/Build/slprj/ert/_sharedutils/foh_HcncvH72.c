...
void foh_HcncvH72(const real_T X_0[100], const real_T U[30], real_T s, const
                  ...
                  real_T AR[100])
{
  ...
  for (k = 0; k < 9; k++) {
    ...
    for (k_ = 0; k_ < 9; k_++) {
      ...
      deriv_iDMkweTP(tspan[k_], P0, u, tspan, s, J, k1);
      a = h / 2.0;
      for (i = 0; i < 190; i++) {
        P0_0[i] = a * k1[i] + P0[i];
      }

      deriv_iDMkweTP(h / 2.0 + tspan[k_], P0_0, u, tspan, s, J, k2);
      a = h / 2.0;
      for (i = 0; i < 190; i++) {
        P0_0[i] = a * k2[i] + P0[i];
      }

      deriv_iDMkweTP(h / 2.0 + tspan[k_], P0_0, u, tspan, s, J, k3);
      for (i = 0; i < 190; i++) {
        P0_0[i] = h * k3[i] + P0[i];
      }

      deriv_iDMkweTP(tspan[k_] + h, P0_0, u, tspan, s, J, k4);
      h /= 6.0;
      for (i = 0; i < 190; i++) {
