...
/* Function for MATLAB Function: '<S342>/soar_INTERP' */
void deriv_3jC2Hyd6(real_T t, const real_T x[10], const real_T u[30], const
                    real_T ut[10], const real_T J[9], real_T dx[10])
{
  ...
  if (mag < 1.0E-5) {
            ...
            if ((!(t > b_x[9])) && (!(t < b_x[0]))) {
              ...
              if (mag == 0.0) {
                ...
                if (scale == varargin_2[n]) {
                  ...
                  u_t[0] = (1.0 - mag) * scale + mag * varargin_2[n];
                }
    ...
    for (n = 0; n < 3; n++) {
      ...
      tmp_0[1 + offset] = ((real_T)b[3 * n] * mag + tmp[3 * n]) * 0.5;
      c_k = 3 * n + 1;
      tmp_0[2 + offset] = ((real_T)b[c_k] * mag + tmp[c_k]) * 0.5;
      c_k = 3 * n + 2;
      tmp_0[3 + offset] = ((real_T)b[c_k] * mag + tmp[c_k]) * 0.5;
    }
