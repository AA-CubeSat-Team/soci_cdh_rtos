...
void com_task(void *pvParameters)
{
	...
	if (kStatus_Success != LPUART_RTOS_Send(&LPUART4_rtos_handle, (uint8_t *)send_message, strlen(send_message)))
	{
		vTaskSuspend(NULL);
	}
	...
	do
	{
		error = LPUART_RTOS_Receive(&LPUART4_rtos_handle, recv_buffer, sizeof(recv_buffer), &n);
		if (error == kStatus_LPUART_RxHardwareOverrun)
		{
			/* Notify about hardware buffer overrun */
			if (kStatus_Success !=
				LPUART_RTOS_Send(&LPUART4_rtos_handle, (uint8_t *)hardware_overrun, strlen(hardware_overrun)))
			{
				vTaskSuspend(NULL);
			}
		}
		if (error == kStatus_LPUART_RxRingBufferOverrun)
		{
			...
			if (kStatus_Success != LPUART_RTOS_Send(&LPUART4_rtos_handle, (uint8_t *)ring_overrun, strlen(ring_overrun)))
			{
				vTaskSuspend(NULL);
			}
		}
		if (n > 0)
		{
			...
			if (kStatus_Success != LPUART_RTOS_Send(&LPUART4_rtos_handle, recv_buffer, n))
			{
				vTaskSuspend(NULL);
			}
		}
