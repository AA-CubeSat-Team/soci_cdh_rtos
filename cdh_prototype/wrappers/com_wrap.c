...
static bool enterCommandMode()
 {
	...
	size_t n = 0;
	int returnVal =  LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)tx_buffer, 3); //LPUART_RTOS_Send(&uart2_handle, (uint8_t *)tx_buffer, 3);
    PRINTF("message sent\n");
...
static bool sendConfigCommand(uint8_t data[], uint8_t expectedResponse[], int sizeofTx, int sizeExpectedResponse) {
    ...
    while (try < DEFAULT_RETRIES) {
    	...
    	int sendReturnVal = LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)tx_buffer, sizeofTx); //LPUART_RTOS_Send(&uart2_handle, tx_buffer, sizeofTx); // Rithu: changing to sizeOfTx
...
void com_getCommands() //highest priority
{
	...
	if (kStatus_Success != LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)to_send, strlen(to_send)))//LPUART_RTOS_Send(&uart2_handle, (uint8_t *)to_send, strlen(to_send)))
	{
	...
	if (n > 0)
	{
		...
		if (kStatus_Success != LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)rcv_buffer, n))//LPUART_RTOS_Send(&uart2_handle, (uint8_t *)rcv_buffer, n))
		{
			vTaskSuspend(NULL);
		}
	}
