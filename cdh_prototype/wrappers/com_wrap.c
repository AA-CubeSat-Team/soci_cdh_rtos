...
static bool enterCommandMode()
 {
	...
	size_t n = 0;
	int returnVal =  LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)tx_buffer, 3); //LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)tx_buffer, 3);
    PRINTF("message sent\n");
...
static bool sendConfigCommand(uint8_t data[], uint8_t expectedResponse[], int sizeofTx, int sizeExpectedResponse) {
    ...
    while (try < DEFAULT_RETRIES) {
    	...
    	int sendReturnVal = LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)tx_buffer, sizeofTx); //LPUART_RTOS_Send(&LPUART3_rtos_handle, tx_buffer, sizeofTx); // Rithu: changing to sizeOfTx
        ...
        int recReturnVal = LPUART_RTOS_Receive(&LPUART3_rtos_handle, rx_buffer, sizeExpectedResponse, size_t);
...
void com_getCommands() //highest priority
{
	...
	if (kStatus_Success != LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)to_send, strlen(to_send)))//LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)to_send, strlen(to_send)))
	{
		PRINTF("could not send!!!\r\n\r\n");
		return;
	}

	status_t error = LPUART_RTOS_Receive(&LPUART3_rtos_handle, rcv_buffer, sizeof(rcv_buffer), &n);
	...
	if (n > 0)
	{
		...
		if (kStatus_Success != LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)rcv_buffer, n))//LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)rcv_buffer, n))
		{
			vTaskSuspend(NULL);
		}
	}
...
//	    }
	    ...
	    int n = 0;
	    status_t error = LPUART_RTOS_Receive(&LPUART3_rtos_handle, receive_buffer, 8, &n);
	    do
	    {
	    	error = LPUART_RTOS_Receive(&LPUART3_rtos_handle, receive_buffer, 8, &n);
	        ...
	        {
	            /* send back the received data */
	            if (kStatus_Success != LPUART_RTOS_Send(&LPUART3_rtos_handle, (uint8_t *)receive_buffer, 8))
	            {
	                vTaskSuspend(NULL);
	            }
	        }
