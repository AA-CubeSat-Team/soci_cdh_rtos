...
//#endif
	...
	rx = norm2(w->c, w->n); w->resx0 = MAX1(1, rx);
	ry = norm2(w->b, w->p); w->resy0 = MAX1(1, ry);
	rz = norm2(w->h, w->m); w->resz0 = MAX1(1, rz);
...
void updateStatistics(pwork* w)
{
    ...
    /* residuals */
    nry = w->p > 0 ? norm2(w->ry, w->p)/MAX1(w->resy0+w->nx,1) : 0.0;
    nrz = norm2(w->rz, w->m)/MAX1(w->resz0+w->nx+w->ns,1);
	info->pres = MAX1(nry, nrz) / w->tau;
	info->dres = norm2(w->rx, w->n)/MAX1(w->resx0+w->ny+w->nz,1) / w->tau;
     ...
     */
    info->pinfres = (w->hz + w->by)/MAX1(w->ny+w->nz,1) < -w->stgs->reltol ? w->hresx / MAX1(w->ny+w->nz,1) : ECOS_NAN;
    info->dinfres = w->cx/MAX1(w->nx,1) < -w->stgs->reltol ? MAX1(w->hresy/MAX1(w->nx,1), w->hresz/MAX1(w->nx+w->ns,1)) : ECOS_NAN;
