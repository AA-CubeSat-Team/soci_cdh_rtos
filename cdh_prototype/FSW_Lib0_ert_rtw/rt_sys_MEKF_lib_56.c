 ...
 */
void IfWarningError(const real_T rtu_dcm[9], DW_IfWarningError *localDW,
                    P_IfWarningError *localP, real_T rtp_action, real_T
                    rtp_tolerance)
{
  boolean_T tmp;
  ...
  for (i = 0; i < 3; i++) {
    for (i_0 = 0; i_0 < 3; i_0++) {
      localDW->rtu_dcm[i + 3 * i_0] = ((rtu_dcm[3 * i + 1] * rtu_dcm[3 * i_0 + 1]
  ...
  for (i = 0; i < 9; i++) {
     ...
     */
    rtb_Compare_f[i] = (fabs(localDW->rtu_dcm[i]) > rtp_tolerance);
  }
  ...
  if (fabs((((((rtu_dcm[0] * rtu_dcm[4] * rtu_dcm[8] - rtu_dcm[0] * rtu_dcm[5] *
            ...
            - rtu_dcm[2] * rtu_dcm[4] * rtu_dcm[6]) + localP->Bias_Bias) >
      rtp_tolerance) {
    ...
    if (tmp) {
      ...
      if (rtp_action == 2.0) {
         ...
         */
        utAssert(localP->Constant1_Value_f != 0.0);
